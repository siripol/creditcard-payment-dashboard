...
2. For each, propose a category **from the valid list** using your own knowledge of the merchant
   (e.g. `SIAM PARAGON` → `Shopping & Retail`, `MUANG THAI LIFE` → `Insurance`). No external API is
...
3. **Show the proposed keyword → category table and ask the user to confirm or edit** before
   writing anything.
4. On confirmation, upsert the entries into `merchant_category.json`, rebuild, and report what
   changed.
