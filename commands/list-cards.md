---
description: List every credit card — display name, last-5 digits, and cycle/expiry date (mmdd).
...
1. **Gather the card set.** The authoritative list of detected cards is `CCDATA.cardMeta` in
   `skills/credit-card-spending-dashboard/data.js` (each key = the card's last 5 digits, with a
...
5. Note under the table that `mmdd` is the statement-cycle date (anchor month + closing day), not
   the physical card expiry. To change one, use `/set-expiryCard <last5> <mmdd>`.
