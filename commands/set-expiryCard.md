---
description: Set a credit card's statement-cycle date (cycle anchor month + closing day) by its last 4 digits.
argument-hint: <last4> <mmdd>
---

Update the credit-card dashboard config for card `$1` with cycle date `$2`.

Argument reference: `$1` = the last 4 digits of the card. `$2` = `MMDD`, where `MM` is the
cycle anchor month (the month the rewards/accumulation cycle resets on) and `DD` is the
statement closing day.

Do this:

1. Validate the arguments. `$1` must be exactly 4 digits. `$2` must be 4 digits; the first
   two (`MM`) must be `01`–`12`. If either is invalid, stop and explain the correct format
   (`/set-expiryCard 1234 1015`).
