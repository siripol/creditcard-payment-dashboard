...
exact name. Rules are written to **`skills/credit-card-spending-dashboard/merchant_group.json`
...
2. **Resolve the exact merchant.** Match `<merchant>` against the distinct `CCDATA.tx[].merch`
   values in `skills/credit-card-spending-dashboard/data.js` (run `python3 build_data.py` from that
